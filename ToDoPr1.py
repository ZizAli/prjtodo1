...
class TaskIO:
    ...
    def write_tasks_to_csv(self, tasks, filename):
        try:
            with open(filename, mode='a', newline='') as file:  # Changed 'w' to 'a'
...
def main():
    ...
    #             reader = csv.DictReader(file)
    #             for row in reader:
    #                 print("Row keys:", row.keys())  # Add this line to check the keys in each row
    #                 task = Task(
    #                     description=row['Description'],
    #                     severity=Severity[row['Severity'].upper()],
    #                     deadline=Weekday[row['Deadline (Weekday)'].upper()],
    ...
    while True:
        ...
        if task_manager.tasks:
            TaskIO().write_tasks_to_csv([task], csv_file_name)  # Write the new task only
